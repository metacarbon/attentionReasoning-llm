...
from utils import load, TASK_NAME_MAPPING, SUBJECTS, choices, SIQA_choices
...
def extract_choice(gen, choice_list):
    res = re.search(
        r"(?:(?:[Cc]hoose)|(?:(?:[Aa]nswer|[Cc]hoice)(?![^ABC]{0,20}?(?:n't|not))[^ABC]{0,10}?\b(?:|is|:|be))\b)[^ABC]{0,20}?\b(A|B|C)\b",
        gen,
    )

    if res is None:
        res = re.search(
            r"\b(A|B|C)\b(?![^ABC]{0,8}?(?:n't|not)[^ABC]{0,5}?(?:correct|right))[^ABC]{0,10}?\b(?:correct|right)\b",
            gen,
        )

    if res is None:
        res = re.search(r"^(A|B|C)(?:\.|,|:|$)", gen)

    if res is None:
        res = re.search(r"(?<![a-zA-Z])(A|B|C)(?![a-zA-Z=])", gen)
...
def extract_answer(response, row):
    gen = process_before_extraction(response, {choice: row[f"answer{choice}"] for choice in SIQA_choices})
    pred = extract_choice(gen, [row[f"answer{choice}"] for choice in SIQA_choices])
    return pred
...
def eval_siqa(
    ...
    **kwargs
    ...
    if save_result_dir:
        ...
        result_path = os.path.join(save_result_dir, "llama13b_chat_cot_siqa_predictions.csv")
        responses_path = os.path.join(save_result_dir, "llama13b_chat_cot_siqa_responses.csv")
    else:
        result_path = None
        responses_path = None
    ...
    if save_result_dir:
        ...
        with open(result_path, 'w', encoding='utf-8') as f:
            ...
            for pred in result:
                pred_numeric = SIQA_choices.index(pred) + 1  # Convert A/B/C to 1/2/3
...
def main(args):
    ...
    eval_siqa(
        ...
        tokenizer,
        test_df,
        save_result_dir="outs_chat/siqa_eval_result",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="/sata/data/LLMs/Llama-2-13b-chat-hf/", 
    )
